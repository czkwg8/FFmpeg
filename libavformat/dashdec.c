...
static int reopen_demux_for_component(AVFormatContext *s, struct representation *pls)
{
    ...
    if (c->cenc_decryption_keys)
        av_dict_set(&in_fmt_opts, "decryption_keys", c->cenc_decryption_keys, AV_OPT_FLAG_DECODING_PARAM);
...
static const AVOption dash_options[] = {
    ...
    { "cenc_decryption_key", "Media default decryption key (hex)", OFFSET(cenc_decryption_key), 
        AV_OPT_TYPE_STRING, {.str = NULL}, INT_MIN, INT_MAX, .flags = FLAGS },
    { "cenc_decryption_keys", "Media decryption keys by KID (hex)", OFFSET(cenc_decryption_keys), 
        AV_OPT_TYPE_STRING, {.str = NULL}, INT_MIN, INT_MAX, .flags = FLAGS },
    {NULL}
};
